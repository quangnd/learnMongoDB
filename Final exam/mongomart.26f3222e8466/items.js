...
function ItemDAO(database) {
    ...
    this.db.collection('item').aggregate([{ 
            ...
            for(var i=0; i < categories.length; i++) {       
                countItem += categories[i].num;
            }

            categories.unshift({ _id: "All", num: countItem});
    ...
    this.getItems = function(category, page, itemsPerPage, callback) {
        ...
        var query;
        if (category !== "All") {
            query = this.db.collection('item').find({category: category}).sort({_id:1}).skip(page * itemsPerPage).limit(itemsPerPage);
        }
        else {
            query = this.db.collection('item').find().sort({_id:1}).skip(page * itemsPerPage).limit(itemsPerPage);
        }

        console.log(query);
        query.toArray(function(err,pageItems){
    ...
    this.getNumItems = function(category, callback) {
         ...
         */
         var query;
         if (category !== "All") {
            query = this.db.collection('item').aggregate([ 
         ...
         query.toArray(function(err, categories) {
            ...
            var numItems = 0;
            if (category !== "All") 
                numItems = categories[0].num;
            else {
                for(var i=0; i < categories.length; i++) {       
                 numItems += categories[i].num;
               }
            }
    ...
    this.searchItems = function(query, page, itemsPerPage, callback) {
        ...
        this.db.collection('item').find({$text: {$search: query}}, {score: {$meta: "textScore"}}).sort({_id:1}).skip(page * itemsPerPage).limit(itemsPerPage).toArray(function(err,items){
    ...
    this.getNumSearchItems = function(query, callback) {
        ...
        */
         ...
         this.db.collection('item').find({$text: {$search: query}}, {score: {$meta: "textScore"}}).sort({_id:1}).toArray(function(err,items){
    ...
    this.addReview = function(itemId, comment, name, stars, callback) {
        ...
        // this.db.collection('item').find({_id: itemId}).toArray(function(err,items){
        //     console.log("Fuck:" + items[0].title + "reviewDoc:" + reviewDoc.toString());
           
        //     items[0].reviews = [reviewDoc];
        //     callback(items[0]);
        // })
